...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/dbchaincloud/cosmos-sdk/types"
	godoc -http=:6060
...
test-ledger-mock:
	@go test -mod=readonly `go list github.com/dbchaincloud/cosmos-sdk/crypto` -tags='cgo ledger test_ledger_mock'

test-ledger: test-ledger-mock
	@go test -mod=readonly -v `go list github.com/dbchaincloud/cosmos-sdk/crypto` -tags='cgo ledger'
...
format: tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/dbchaincloud/cosmos-sdk
.PHONY: format
...
devdoc-init:
	docker run -it -v "$(CURDIR):/go/src/github.com/dbchaincloud/cosmos-sdk" -w "/go/src/github.com/dbchaincloud/cosmos-sdk" tendermint/devdoc echo
	# TODO make this safer
	$(call DEVDOC_SAVE)

devdoc:
	docker run -it -v "$(CURDIR):/go/src/github.com/dbchaincloud/cosmos-sdk" -w "/go/src/github.com/dbchaincloud/cosmos-sdk" devdoc:local bash
